...
county_map_file = open(os.path.join(
    map_dir, 'County.geojson'), encoding='utf-8').read()
...
def home(request):
    ...
    # map
    m = folium.Map(location=[23.97565, 120.9738819],
                   zoom_start=8, height="100%", position="initial")
                ...
                for tag in tag_list:
                    if tag.imgur_url:
                        icon = folium.features.CustomIcon(
                            tag.imgur_url, icon_size=(80, 80))
                        m2 = folium.Marker(
                            location=[post.lat, post.lng], popup=popup, icon=icon, color=color)
                        icon_config = True
                        break
            if not icon_config:
                if post.category == 1:
...
def tag(request, pk):
    ...
    post = Post.objects.filter(tags=pk, permanently_closed=False).order_by(
        '-stars', '-published_date')
...
def area(request):
    ...
    for s in url:
        ...
        if s == '=':
            ...
            area_ID += str(s)
        count += 1
    post = Post.objects.filter(area=area_ID, permanently_closed=False).order_by(
        '-stars', '-published_date')
...
def attraction(request):
    post = Post.objects.filter(category=1, permanently_closed=False).order_by(
        '-stars', '-published_date')
    my_filter = PostFilter(request.GET, queryset=post)
    post = my_filter.qs
    user = request.user
...
def accomodation(request):
    post = Post.objects.filter(category=2, permanently_closed=False).order_by(
        '-stars', '-published_date')
    my_filter = PostFilter(request.GET, queryset=post)
    post = my_filter.qs
    user = request.user
...
def restaurant(request):
    post = Post.objects.filter(category=3, permanently_closed=False).order_by(
        '-stars', '-published_date')
    my_filter = PostFilter(request.GET, queryset=post)
    post = my_filter.qs
    user = request.user
...
def post_detail(request, pk):
    ...
    popup = '<div class="popup"><a href="/post/' + str(post.pk) + '" target="_parent" style="text-decoration: none; color:#000"><h1>' + post.title + '</h1>'\
            ...
            + '<h4>想去指數: ' + stars + '</h4>'
    if post.imgur_url:
        popup = popup + '<img src="' + post.imgur_url + \
